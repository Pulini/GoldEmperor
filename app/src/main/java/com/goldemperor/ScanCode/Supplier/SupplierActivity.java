package com.goldemperor.ScanCode.Supplier;
...
import com.goldemperor.ScanCode.Share.URLCode;
...
public class SupplierActivity extends Activity implements ScanListener {
    ...
    private void setListener() {
        ...
        B_UpData.setOnClickListener(v -> SU.GetBarCodeStatus(ScanUtil.GetBarCodeType_FCgInStockBillID));
        ...
        B_Submit.setOnClickListener(v -> {
            if (CDM.size() == 0) {
                ...
                if(SU.CheckList(CDM)){
                    ...
                    SU.GetReport(list, ScanUtil.BillTypeID_Supplier, false);
                }else{
    ...
    private void initdata() {
        ...
        try {
            cdm = mGson.fromJson(SU.get(define.BarCode_SA, ""), new TypeToken<List<CodeDataModel>>() {
            }.getType());
        ...
        SU.GetBarCodeStatus(ScanUtil.GetBarCodeType_FCgInStockBillID);
    }
    ...
    private void Submit() {
        ...
        map.put("barcodeJson", URLCode.toURLEncoded(mGson.toJson(list)));
        map.put("OrganizeID", SU.OrganizeID);
        map.put("BillTypeID", ScanUtil.BillTypeID_Supplier + "");
        map.put("TranTypeID", ScanUtil.TranTypeID_Supplier_Warehousing+"");
        ...
        WebServiceUtils.callWebService(
                ...
                define.SubmitBarCode2CollectBill,
                map, result -> {
                    mProgressHUD.dismiss();
                    if (result != null) {
                        try {
                            result = URLDecoder.decode(result, "UTF-8");
                            LOG.E("供应商扫码入库提交=" + result);
        ...
        if (ReturnType) {
            ...
            if (FBCM.size() > 0) {
                ...
                if (CDM.size() > 0) {
                    ...
                    SA.Updata(CDM);
                    SU.seve(define.BarCode_SA, mGson.toJson(CDM));
                }
            }
        } else {
